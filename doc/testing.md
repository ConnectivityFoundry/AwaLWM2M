...
### Awa API Testing
 ...
 It is possible to run individual tests for debugging purposes, using the --gtest_filter flag. Wildcards (*) can be passed in order to run multiple tests/suits that match the filter. (eg. --gtest_filter=TestGet.\* to run all tests in the TestGet suite). Refer to the gtest advanced guide for further details.
 ...
 Running the tests with Valgrind is highly recommended to ensure no memory leaks are introduced.
...
### Awa Tools Testing
 ...
 Tests are provided that run against the supplied LWM2M tools. These tests help ensure that the tools perform correctly and produce the expected output, including error codes. These tests are written in Python and depend on the nosetests package.
...
### SDK Testing
 ...
 Multiple tests can be run by separating them with a colon character:
